  ...
  const getAllDoctors = async () => {
    ...
    try {
      const response = await axios({
        method: 'GET',
        url: ApiConfig.upcomingListDoctor,
        headers: {
      ...
      if (response?.data?.error === 'false') {
        ...
        console.log('asgdfdsdfh', response?.data?.data);
        // const today = new Date();
        // today.setHours(0, 0, 0, 0); // start of today

        // const filteredData = allData.filter((item) => {
        //   const itemDate = new Date(item?.createdAt); // update `item.date` if different
        //   itemDate.setHours(0, 0, 0, 0);
        //   return itemDate < today; // only include data before today
        // });
